        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col">
            ...
            <div class="relative flex-1" style="min-height: 180px;">
                ...
                <div class="absolute inset-0 flex items-end gap-1.5 pl-10 pb-7">
                    @php
                        $maxCal = max(collect($weekly)->pluck('calories')->max(), $target * 1.1, 100);
                    @endphp
                    @foreach($weekly as $day)
                    @php
                        $pct = $maxCal > 0 ? min(100, ($day['calories'] / $maxCal) * 100) : 0;
                        $targetPct = $maxCal > 0 ? min(100, ($target / $maxCal) * 100) : 0;
                        ...
                        if ($isOver) {
                            $barColor = 'bg-red-400';
                            $barColorHover = 'hover:bg-red-500';
                        } elseif ($isToday) {
                            $barColor = 'bg-brand-500';
                            $barColorHover = 'hover:bg-brand-600';
                        } else {
                            $barColor = $isEmpty ? 'bg-gray-100' : 'bg-brand-300';
                            $barColorHover = $isEmpty ? '' : 'hover:bg-brand-400';
                        }
                    @endphp
                    <div class="flex-1 flex flex-col items-center gap-1 h-full justify-end group relative"
                        ...
                        <div class="w-full rounded-t-lg transition-all duration-700 ease-out {{ $barColor }} {{ $barColorHover }} cursor-default"
                             style="height: {{ max($pct, $isEmpty ? 2 : 4) }}%; min-height: {{ $isEmpty ? '2px' : '4px' }};">
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                {{-- Target dashed line --}}
                @php
                    $targetLinePct = isset($maxCal) && $maxCal > 0 ? min(100, ($target / $maxCal) * 100) : 0;
                @endphp
                <div class="absolute left-10 right-0 pointer-events-none"
                    ...
                    <div class="border-t-2 border-dashed border-orange-400 w-full relative">
                        <span class="absolute -top-4 right-0 text-[9px] font-semibold text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded">
                            Target
                        </span>
                    </div>
                </div>
            ...
            <div class="mt-5 pt-4 border-t border-gray-100 flex items-center gap-4 flex-wrap">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-[11px] text-gray-500">Normal</span>
                </div>
                <div class="flex items-center gap-1.5">
                    <div class="w-3 h-3 rounded-sm bg-red-400"></div>
                    <span class="text-[11px] text-gray-500">Melebihi target</span>
                </div>
                <div class="flex items-center gap-1.5">
                    <div class="w-4 h-0 border-t-2 border-dashed border-orange-400"></div>
                    <span class="text-[11px] text-gray-500">Target</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="py-3 flex items-center gap-4">
                @if($log->image_path)
                <img src="{{ $log->image_path }}" class="w-12 h-12 rounded-xl object-cover flex-shrink-0">
                @else
                <div class="w-12 h-12 rounded-xl bg-brand-50 flex items-center justify-center text-xl flex-shrink-0">
                    🍴
                </div>
                @endif
                <div class="flex-1 min-w-0">
